...
class ResourceFileMergerTask extends CommonPropertyFileMergerTask {
	...
	def taskAction() {
		...
		mergePropertyFiles(project, resourceFilters, "${project.buildDir}/packaging/resources","${project.buildDir}/template/app/conf/ops","${project.projectDir}/${servicePropertiesDir}",'resource')
	}
}
...
class AppConfigFileMergerTask extends CommonPropertyFileMergerTask {
	...
	def taskAction() {
		...
		def servicePropertiesDir = project.extensions.apgPackage.resourcesPath
		mergePropertyFiles(project, appConfigFilters,"${project.buildDir}/packaging/appconfigs","${project.buildDir}/template/app/conf/app","${project.projectDir}/${servicePropertiesDir}",'appconfig')
	}
}
