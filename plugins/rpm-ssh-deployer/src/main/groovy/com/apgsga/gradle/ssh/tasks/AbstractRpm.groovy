...
abstract class AbstractRpm extends DefaultTask {
    ...
    static def preConditions(def apgRpmDeployConfigExt, def apgSshCommonExt) {
        ...
        Assert.notNull(apgRpmDeployConfigExt.rpmFilePath, "${ApgRpmSshDeployer.APG_RPM_DEPLOY_CONFIG_EXTENSION_NAME} requires a rpmFilePath to be configured" as Supplier<java.lang.String>)
        Assert.notNull(apgRpmDeployConfigExt.rpmFileName, "${ApgRpmSshDeployer.APG_RPM_DEPLOY_CONFIG_EXTENSION_NAME} requires a rpmFileName to be configured" as Supplier<String>)
        Assert.notNull(apgRpmDeployConfigExt.remoteDestFolder, "${ApgRpmSshDeployer.APG_RPM_DEPLOY_CONFIG_EXTENSION_NAME} requires a remoteDestFolder to be configured" as Supplier<String>)
    }
